...
if ( ! function_exists( 'easydocs_setup' ) ) :
/**
 ...
 */
function easydocs_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 */
	load_theme_textdomain( 'easydocs', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'header-menu' => esc_html__( 'Header Menu', 'easydocs' ),
	) );
...
endif; // easydocs_setup
add_action( 'after_setup_theme', 'easydocs_setup' );
 ...
 */
function easydocs_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'easydocs_content_width', 640 );
}
add_action( 'after_setup_theme', 'easydocs_content_width', 0 );
 ...
 */
function easydocs_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'easydocs' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'easydocs_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function easydocs_scripts() {
	wp_enqueue_style( 'easydocs-style', get_stylesheet_uri() );

	wp_enqueue_script( 'easydocs-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20120206', true );

	wp_enqueue_script( 'easydocs-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'easydocs_scripts' );
 ...
 */
function easydocs_change_style_version_num($styles) {
	$styles -> default_version = "123";
}
//add_action("wp_default_styles", "easydocs_change_style_version_num");

/***
 * Hierarchy Functions
 ***/
// Walk the hierarchy and return an array of IDs
if (!function_exists('easydocs_get_hierarchy')) {
	function easydocs_get_hierarchy() {
		global $post;
		$hierarchy = array();
		if($post->ID) {
			$hierarchy[] = $post->ID;
...
if (!function_exists('easydocs_get_root_map_id')) {
	function easydocs_get_root_map_id() {
 ...
 ***/
if (!function_exists('easydocs_get_subsections')) {
	function easydocs_get_subsections() {
		global $post;
 ...
 **/
if (!function_exists('easydocs_get_the_shortdesc')) {
	function easydocs_get_the_shortdesc() {
		global $post;
...
if (!function_exists('easydocs_get_404_content')) {
	function easydocs_get_404_content() {
		?>
		<div class="home-search">
			<div class="header"><?php echo get_theme_mod( '404_header', __('Oops! That page can&rsquo;t be found.', 'easydocs') ); ?></div>
...
**/
function easydocs_insert_ga_info() {
	echo "<script>
...
add_action('wp_head','easydocs_insert_ga_info');

/**
 * Get the list of available skins
**/
if (!function_exists('easydocs_get_all_skins')) {
	function easydocs_get_all_skins() {
 ...
 **/
function easydocs_add_skin_stylesheet() {
...
add_action('wp_head','easydocs_add_skin_stylesheet');
